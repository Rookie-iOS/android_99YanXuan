...
public class RushBuyModel extends BaseModel {
    private RushBuyPresenter presenter;
    public RushBuyModel(BasePresenter basePresenter) {
        super(basePresenter);
        presenter = (RushBuyPresenter) basePresenter;
    }

    public void loadData(String shopId){
        HttpHelper.bulid().getRequest(HttpUrl.build().getRushbuy(shopId),
